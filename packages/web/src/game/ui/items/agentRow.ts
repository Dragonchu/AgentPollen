...
export function buildAgentRow(
  ...
  const bg = rexUI.add.roundRectangle(
    ...
    setInteractive(config?: object): typeof bg;
    on(event: string, cb: () => void): typeof bg;
  };
  ...
  const statsText = scene.add.text(0, 0, `${agent.killCount}💀 ${Math.round(agent.hp)}hp`, {
