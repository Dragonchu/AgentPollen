...
function VoteBar({
  ...
  const barColor =
    agent.color === 'blue' ? 'from-primary/80 to-primary' : 'from-accent/80 to-accent';
  const glowColor =
    agent.color === 'blue'
    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          <div
            className={`w-1.5 h-1.5 rounded-full ${agent.color === 'blue' ? 'bg-primary' : 'bg-accent'} animate-pulse-neon`}
          />
          <span className="font-mono text-xs text-foreground tracking-wide">{agent.name}</span>
        </div>
        <div className="flex items-center gap-2">
          <span
            className={`font-mono text-[10px] ${agent.trend > 0 ? 'text-emerald-400' : 'text-red-400'}`}
          >
            {agent.trend > 0 ? '+' : ''}
...
export function LiveVotingDashboard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setDataStreams((prev) =>
        prev.map((s) => ({ ...s, progress: s.progress + 0.1 })).filter((s) => s.progress < 1),
      );
      setRipples((prev) =>
        prev.map((r) => ({ ...r, progress: r.progress + 0.25 })).filter((r) => r.progress < 1),
      );
    }, 100);
    return () => clearInterval(interval);
  }, [mounted]);
    ...
    >
      ...
      <div className="relative px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs font-medium uppercase tracking-[0.15em] text-foreground">
              Live Voting
            </span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 p-3 border-b border-border/20">
        ...
        <StatBox
          icon={Activity}
          label="Active"
          value={`${agents.filter((a) => a.trend > 0).length * 12 + 4}`}
        />
      </div>
