      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-lg border border-border/30 bg-background/40 backdrop-blur-sm overflow-hidden">
          ...
          <div className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-border/20 bg-background/30">
            <span className="col-span-1 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
              #
            </span>
            <span className="col-span-3 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
              Agent
            </span>
            <span className="col-span-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider text-center hidden sm:block">
              Eliminations
            </span>
            <span className="col-span-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider text-center hidden sm:block">
              Survived
            </span>
            <span className="col-span-2 sm:col-span-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider text-right">
              Votes
            </span>
            <span className="col-span-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider text-center">
              Trend
            </span>
          </div>
            ...
            return (
              ...
              >
                <div className="col-span-1 flex items-center">
                  <span
                    className={`font-mono text-sm ${isTop3 ? 'text-accent font-bold' : 'text-muted-foreground'}`}
                  >
                    {entry.rank}
                  </span>
                </div>
                <div className="col-span-5 sm:col-span-3 flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${entry.color === 'blue' ? 'bg-primary' : 'bg-accent'}`}
                  />
                  <span className={`font-mono text-sm ${nameColor} tracking-wide`}>
                    {entry.name}
                  </span>
                </div>
